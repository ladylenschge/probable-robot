...
ipcMain.handle('update-schedule-slot', async (e, slot: IDailyScheduleSlot) => {
    ...
    try {
        ...
        if (!updatedSlot) {
            throw new Error("Could not find the updated slot after database transaction.");
        }
        return updatedSlot;
    } catch (err) {
        dbRun('ROLLBACK');
